...
export default function Slideshow({ photos, mode, theme }) {
    ...
    const changePhoto = useCallback((newIndex) => {
        ...
        // After animation duration, change photo and reset
        setTimeout(() => {
            setCurrentIndex(newIndex);
            setIsTransitioning(false);
        }, theme==="theme-d" ? 0 : 800); // Match your CSS animation duration
    }, [isTransitioning]);
   ...
   useEffect(() => {
    if(photoStack.length >= photos.length) setPhotoStack((prev) => [...prev.slice(1)]); 
        setPhotoStack(prev => {
    ...
    return (
        <div>
            <div ref={slideshowRef} className={`slideshow ${theme} ${isFullscreen ? 'fullscreen' : ''}`}>
               {(theme === "theme-c" || theme === "theme-b" || theme === "theme-a" || theme === "theme-f") && <PhotoItem 
                ...
                {theme === "theme-d" && photoStack.map((photo, index) => (
                    <PhotoItem
                        key={photo.fileUrl}
                        photo={photo}
                        theme={theme}
                        ...
                        animationClass={index === currentIndex ? (isTransitioning ? 'exit' : 'enter') : ''}
                    />
                ))}
                ...
                {theme === "theme-e" && 
                    (
                      ...
                      <div className="current-photo">
                        <div className={`first-half ${isTransitioning ? 'exit' : 'enter'}`}
                            style={{
                                backgroundImage: `url(${currentPhoto.fileUrl})`,
                            }}
                        ></div>
                        <div className={`second-half ${isTransitioning ? 'exit' : 'enter'}`}
                            style={{
                                backgroundImage: `url(${currentPhoto.fileUrl})`,
                            }}
                        ></div>
           ...
           {mode === "manual" && (
            <>
             <button onClick={prevPhoto} disabled={isTransitioning} className='manual-handle prev'> &larr; prev</button>
            <button onClick={nextPhoto} disabled={isTransitioning} className='manual-handle next'>next &rarr;</button>
            </>
           )}
            <button onClick={handleFullscreen} style={{marginLeft:8}} 
